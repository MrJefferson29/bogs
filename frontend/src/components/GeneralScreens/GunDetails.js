...
export default function GunDetails() {
  ...
  const handleEmailClick = () => {
    ...
    const body =
      `Dear BOGS Team,\n\n` +
      `I am interested in purchasing the ${gun.name} from your ${gun.category} collection.\n\n` +
      `Could you please provide more details regarding its availability and shipping?\n\n` +
      `Thank you!\n\n` +
      `[Your Name]`;
  ...
  return (
    <GunDetailsStyles>
      ...
      <div className="details-content">
        ...
        <div className="info">
          <div className="info-left">
            ...
            <p>
              <strong style={{ color: "#ff9900" }}>Subcategory:</strong>{" "}
              {gun.subcategory}
            </p>
            <p>
              <strong style={{ color: "#ff9900" }}>Maker:</strong> {gun.make}
            </p>
            <p>
      ...
      {relatedGuns.length > 0 && (
        <RelatedGunsStyles>
          <h2>Related Firearms</h2>
          <div className="horizontal-scroll">
            {relatedGuns.map((relatedGun, index) => (
              <Link
                key={index}
                to={`/gun/${relatedGun.name.replace(/\s+/g, "-").toLowerCase()}`}
